...
- It has only 2 commands (create and explain)
...
One of the key limiting factor to LLM application is the context size. E.g. the original GPT-3.5 model had a context size of 4k tokens, GPT4 had versions with 8k and 32k context window sizes. Upon approaching models context limit its' performance degrades.
A typical 100 line source code file is around 600-700 tokens. The extension allows picking the context size limit (via settings) and collecting only as much code that can fit. The default value is 4k. 67% in the context window is reserved for prompt tokens (those ones sent to the API) and the rest 33% for completion tokens (tokens generated by LLM, context limit applies to both request and response).
To better understand the boundaries and consumption of tokens the extension has 2 neat features:
1. Displaying status bar counter, that shows the total number of tokens in the currently open file and number of token in selected code block:
<img src="images/statusBar.png" alt="drawing" width="200"/>
2. Displaying prompt and total (prompt + completion) tokens after finishing the request:
<img src="images/timings.png" alt="drawing" width="200"/>
